...
const BusinessType = () => {
    ...
    const showBusinessType = async (pageNo) => {
        ...
        let result = await api('master/business_type/', data)
    ...
    useEffect(() => {
        setCpage(cPage)
        showBusinessType(!param ? param : 1)
    }, [recPerPage])
    ...
    const handleStatus = async (item) => {
        ...
        const result = await api('master/business_type/status', data)
    ...
    return (
        <div>
            <div className="admin_page_top " >
                <h2>BusinessType</h2>
                <Button link='/addBusinessType' iHeight='15px' class='btn-primary' icon={<i class="fa-solid fa-plus"></i>} name='Add' />
            </div>
                    ...
                    {businessType.map((item, index) => {
                        return (
                            <tr>
                                ...
                                <td className='fs-3'>
                                    <Link to={"/businessTypeView/" + item._id}><i class="fa-regular fa-eye"></i></Link>
                                </td>
                            </tr>
                        )
                    })}

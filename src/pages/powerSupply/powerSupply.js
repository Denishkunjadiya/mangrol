...
const PowerSupply = () => {
    ...
    const showPowerSupply = async (pageNo) => {
        ...
        let result = await api('master/power_supply/', data)
    ...
    const handleStatus = async (item) => {
        ...
        const result = await api('master/power_supply/status', data)
    ...
    return (
        <div>
            <div className="admin_page_top " >
                <h2>PowerSupply</h2>
                <Button link='/addPowerSupply' iHeight='15px' class='btn-primary' icon={<i class="fa-solid fa-plus"></i>} name='Add' />
            </div>
                    ...
                    {powerSupply.map((item, index) => {
                        return (
                            <tr>
                                ...
                                <td className='fs-3'>
                                    <Link to={"/powerSupplyView/" + item._id}><i class="fa-regular fa-eye"></i></Link>
                                </td>
                            </tr>
                        )
                    })}

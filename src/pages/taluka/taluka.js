...
const District = () => {
    ...
    const showDistrict = async (pageNo) => {
        ...
        const result = await api('master/taluka', data)
    ...
    const handleStatus = async (item) => {
        ...
        const result = await api('master/taluka/status', data)
    ...
    return (
        <div>
            <div className="admin_page_top " >
                <h2>Taluka</h2>
                <Button link='/addTaluka' iHeight='15px' class='btn-primary' icon={<i class="fa-solid fa-plus"></i>} name='Add' />
            </div>
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th style={{ cursor: "pointer", userSelect: 'none' }} onClick={() => sort("district")}>District {getIcon('district')}</th>
                    ...
                    {taluka.map((item, index) => {
                        return (
                            <tr>
                                ...
                                <td className='fs-3'>
                                    <Link to={"/talukaView/" + item._id}><i class="fa-regular fa-eye"></i></Link>
                                </td>
                            </tr>
                        )
                    })}
